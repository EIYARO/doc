...
## 1、构建交易
API接口 build-transaction，代码[api/transact.go#L117](https://github.com/EIYARO-Project/core/blob/main/api/transact.go#L117)

以标准的非EY资产转账交易为例，资产ID为全F表示EY资产，在该示例中EY资产仅作为手续费，该交易表示花费99个特定的资产到指定地址中。其中构建交易的输入请求json格式如下：
```js
{
...
- 除了EY资产（所有交易都是以EY资产作为手续费）之外，其他资产在构建input和output时，所有输入和输出的资产总和必须相等，否则交易会报出输入输出不平衡的错误信息。
- 交易的手续费： 所有inputs的EY资产数量 - 所有outputs的EY资产数量
- 交易中的资产amount都是neu为单位的，EY的单位换算如下：1 EY = 1000 mEY = 100000000 neu
...
- `AssetAmount` 表示用户需要发行的资产ID和对应的资产数目，这里的`AssetID`需要通过`create-asset`创建，并且这里不能使用`EY`的资产ID
...
例如发行一笔资产的交易示例如下：
（该交易表示发行数量为`900000000`个`assetID`的`42275aacbeda1522cd41580f875c3c452daf5174b17ba062bf0ab71a568c123f`的资产到接收地址`sy1qxe4jwhkekgnxkezu7xutu5gqnnpmyc8ppq98me`中, 其中手续费为`20000000`neu的EY资产）
```js
{
  "base_transaction": null,
  "actions": [
...
例如转账一笔资产的交易示例如下：
（该交易表示通过账户的方式转账`100000000`neu的EY资产到地址`sy1qxe4jwhkekgnxkezu7xutu5gqnnpmyc8ppq98me`中, 其中手续费`20000000`neu = 输入EY资产数量 - 输出EY资产数量）
```js
{
  "base_transaction": null,
  "actions": [
...
（该交易表示通过直接花费UTXO的方式转账`100000000`neu的EY资产到地址`sy1qxe4jwhkekgnxkezu7xutu5gqnnpmyc8ppq98me`中, 其中手续费 = 输入EY资产的UTXO值 - 输出EY资产数量）
```js
{
  "base_transaction": null,
  "actions": [
...
例如转账一笔资产的交易示例如下：
（该交易表示通过账户的方式转账`100000000`neu的EY资产到地址`sy1qxe4jwhkekgnxkezu7xutu5gqnnpmyc8ppq98me`中, 其中`control_address`类型表示以地址作为接收方式）
```js
{
  "base_transaction": null,
  "actions": [
...
例如转账一笔资产的交易示例如下：
（该交易表示通过账户的方式转账`100000000`neu的EY资产到接收`program`（跟`address`是一一对应的）`0014a3f9111f3b0ee96cbd119a3ea5c60058f506fb19`中, 其中`control_program`类型表示以`program`作为接收方式）
```js
{
  "base_transaction": null,
  "actions": [
...
例如销毁一笔资产的交易示例如下：
（该交易表示通过账户的方式将`1`neu的EY资产销毁并添加了附加信息, 其中`retire`表示销毁指定数量的资产）
```js
{
  "base_transaction": null,
  "actions": [
...
结构字段说明如下：
- `TotalNeu` 预估的总手续费（单位为neu），该值直接加到build-transaction的EY资产输入action中即可
- `StorageNeu` 存储交易的手续费
- `VMNeu` 运行虚拟机的手续费
...
## 2、签名交易
API接口 sign-transaction，代码[api/hsm.go#L53](https://github.com/EIYARO-Project/core/blob/main/api/hsm.go#L53)
...
## 3、提交交易
API接口 submit-transaction，代码[api/transact.go#L132](https://github.com/EIYARO-Project/core/blob/main/api/transact.go#L132)
