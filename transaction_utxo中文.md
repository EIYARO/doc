...
以下步骤以及功能改造仅供参考，具体代码实现需要用户根据实际情况进行调试，具体可以参考单元测试案例代码[blockchain/txbuilder/txbuilder_test.go#L255](https://github.com/EIYARO-Project/core/blockchain/txbuilder/txbuilder_test.go#L255)

## 1.创建私钥和公钥
该部分功能可以参考代码[crypto/ed25519/chainkd/util.go#L11](https://github.com/EIYARO-Project/core/crypto/ed25519/chainkd/util.go#L11)，可以通过 `NewXKeys(nil)` 创建主私钥和主公钥 
```go
...
## 2.根据公钥创建接收对象
接收对象包含两种形式：`address`形式和`program`形式，两者是一一对应的，任选其一即可。其中创建单签地址参考代码[account/accounts.go#L267](https://github.com/EIYARO-Project/core/account/accounts.go#L267)进行相应改造为：
```go
...
创建多签地址参考代码[account/accounts.go#L294](https://github.com/EIYARO-Project/core/account/accounts.go#L294)进行相应改造为：(quorum指的是多签地址需要的验证的个数，比如说3-2多签地址，指的是3个主公钥，需要两个签名才能验证通过)
```go
...
## 3.找到可花费的utxo
找到可花费的utxo，其实就是找到接收地址或接收`program`是你自己的`unspend_output`。其中utxo的结构为：（参考代码[account/reserve.go#L39](https://github.com/EIYARO-Project/core/account/reserve.go#L39)）
```go
...
上述这些utxo的字段信息可以从`get-block`接口返回结果的transaction中找到，其相关的结构体如下：（参考代码[api/block_retrieve.go#L26](https://github.com/EIYARO-Project/core/api/block_retrieve.go#L26)）
```go
...
第一步，通过`utxo`构造交易输入`TxInput`和签名需要的数据信息`SigningInstruction`，该部分功能可以参考代码[account/builder.go#L169](https://github.com/EIYARO-Project/core/account/builder.go#L169)进行相应改造为:
```go
...
该部分功能可以参考代码[protocol/bc/types/txoutput.go#L20](https://github.com/EIYARO-Project/core/protocol/bc/types/txoutput.go#L20):
```go
...
通过上面已经生成的交易信息构造交易`txbuilder.Template`，该部分功能可以参考[blockchain/txbuilder/builder.go#L92](https://github.com/EIYARO-Project/core/blockchain/txbuilder/builder.go#L92)进行改造为:
```go
...
## 6.对构造的交易进行签名
账户模型是根据密码找到对应的私钥对交易进行签名，这里用户可以直接使用私钥对交易进行签名，可以参考签名代码[blockchain/txbuilder/txbuilder.go#L82](https://github.com/EIYARO-Project/core/blockchain/txbuilder/txbuilder.go#L82)进行改造为:（以下改造仅支持单签交易，多签交易用户可以参照该示例进行改造）
```go
...
## 7.提交交易上链
该步骤无需更改任何内容，直接参照wiki中提交交易的API[submit-transaction](https://github.com/EIYARO-Project/core/blob/main/API-Reference.md#submit-transaction)的功能即可
